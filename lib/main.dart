...
class _HolidayCalendarPageState extends State<HolidayCalendarPage> {
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      ConstrainedBox(
                        ...
                        child: Text(
                          'Consultez la liste officielle par année, organisée mois par mois à partir de l’API Nager.Date.',
                          textAlign: TextAlign.center,
                          style: theme.textTheme.bodyLarge,
                        ),
                      ),
                    ...
                    LayoutBuilder(
                      builder: (context, constraints) {
                        final columns = constraints.maxWidth > 980
                            ...
                            ? 2
                            : 1;
                        final monthCards = List.generate(12, (index) {
                          final month = index + 1;
                          return _HolidayMonthCard(
                            monthName: _monthLabel(month),
                            holidays: holidaysByMonth[month] ?? const [],
                          );
                        });
                        ...
                        return Wrap(
                          spacing: 16,
                          runSpacing: 16,
                          children: monthCards.asMap().entries
                              .map(
                                (entry) => SizedBox(
                                  width:
  ...
  Future<void> _loadHolidays() async {
    try {
      final results = await HolidayService.fetchHolidays('FR', selectedYear);
...
class _HolidayMonthCard extends StatelessWidget {
  const _HolidayMonthCard({required this.monthName, required this.holidays});
      ...
      child: Column(
        ...
        children: [
          ...
          if (holidays.isEmpty)
            const Text(
              'Aucun jour férié',
              style: TextStyle(color: Color(0xFF60768D)),
            )
          else
            Column(
              children: [
                for (var i = 0; i < holidays.length; i++) ...[
                  ...
                  if (i < holidays.length - 1)
                    const Padding(
                      ...
                      child: Divider(height: 1, color: Color(0xFFE7EDF4)),
                    ),
                ],
              ],
            ),
      ...
      children: [
        Container(
          ...
          child: Column(
            children: [
              Text(
                _weekdayLabel(holiday.date.weekday),
                style: const TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.w700,
                  color: Color(0xFF60768D),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  '${_weekdayLong(holiday.date.weekday)} ${holiday.date.day} ${_monthLabel(holiday.date.month)} ${holiday.date.year}',
                  style: const TextStyle(
                    color: Color(0xFF60768D),
                    height: 1.45,
                  ),
...
class _OfficialSourcesSection extends StatelessWidget {
  const _OfficialSourcesSection();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: const [
          ...
          SizedBox(height: 8),
          Text(
            'Pour vérifier les dates et les règles en complément de l’API utilisée par le site.',
            style: TextStyle(color: Color(0xFF60768D), height: 1.5),
          ),
          SizedBox(height: 18),
          _OfficialLinkCard(
            title: 'Service-Public.fr',
            description:
                'Liste officielle des jours fériés et règles associées.',
            url:
          ...
          _OfficialLinkCard(
            title: 'Service-Public.fr',
            description:
                'Informations officielles sur les ponts et le calendrier annuel.',
            url:
          ...
          _OfficialLinkCard(
            ...
            description:
                'Source technique utilisée pour charger les jours fériés affichés sur la page.',
